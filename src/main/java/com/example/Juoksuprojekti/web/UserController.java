...
import com.example.Juoksuprojekti.domain.SignupForm;
	...
	public String save(@Valid @ModelAttribute("signupform") SignupForm signupForm, BindingResult bindingResult) {
		if (!bindingResult.hasErrors()) { // validation errors
			if (signupForm.getPassword().equals(signupForm.getPasswordCheck())) { // check password match
				...
				if (signupForm.getEmail().contains("@")) { // Check if email is valid
					newUser.setEmail(signupForm.getEmail());
				} else {
					bindingResult.rejectValue("email", "err.email", "Email address is not valid");
					return "signup";
				}
			...
			return "signup"; // return this if no errors
		}
		// ei enää if elseä
